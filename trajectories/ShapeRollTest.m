classdef ShapeRollTest < TrajectoryBase

    methods

        function obj = ShapeRollTest(params)
            obj@TrajectoryBase(params);
        end

        function [sigma, dsigma, ddsigma, dddsigma, ddddsigma] = ...
                get_flat_outputs(obj, t)
            %GET_FLAT_OUTPUTS Generate smooth lateral flat-output trajectory.
            %
            % Flat outputs:
            %   sigma = [x_I; y_I; z_I; psi]
            %
            % Trajectory properties:
            %   - Motion only along inertial y-axis
            %   - x and yaw remain constant
            ...
            %% Acceleration
            ddy = 3 * K * om2 * (1 - c)^2 * (3 * c + 2);

            %% Jerk
            dddy = 3 * K * om3 * s * (1 - c) * (9 * c + 1);

            %% Snap
            ddddy = 3 * K * om4 * (1 - c) * (27 * c^2 + 11 * c - 8);
