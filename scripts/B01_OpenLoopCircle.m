%% Test open-loop for the quadcopter in a Circular Trajectory — B01 model.
%
%  Circular trajectory tracking using differential flatness.
%
%  Trajectory:
%       x(t) = R cos(wt)
%       y(t) = R sin(wt)
...
%
%  Yaw:
%       psi(t) tangent to velocity vector
%
%  Frames:
%       Inertial : NED
%       Body     : FRD
...
quad = B01_ANTX_quadcopter(mp, true);
...
T_final = 1 * tp.T_duration_circle;
...
figure('Name', 'Circle State Tracking', 'Color', 'w', 'Position', [30 50 1600 850]);
...
sgtitle(sprintf('Circle tracking (rigid body)  |  max pos error = %.2e m', max(pos_err)));

%% FIGURE 1b — MOTOR THRUST STATES (13-16), shows the delay-induced lag
T_hover_pm = mp.m * mp.g / 4;
if quad.delay_motors
    figure('Name', 'Circle Motor Thrusts', 'Color', 'w', 'Position', [30 950 1200 350]);
...
figure('Name', 'Circle Inputs', 'Color', 'w', 'Position', [30 50 900 500]);
...
fig3 = figure('Name', '3D Circle Animation', 'Color', 'w', 'Position', [900 100 800 750]);
ax3 = axes(fig3);
PlotDrone2.setup_axes(ax3, '3D Drone — Circular Trajectory (B01)');
hold(ax3, 'on');
