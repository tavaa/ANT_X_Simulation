%% Test open-loop for the quadcopter in a Spiral Ellipse Trajectory.
%
%  Elliptical Spiral trajectory tracking using differential flatness.
%
%  Trajectory:
%       x(t) = Rx cos(wt)
%       y(t) = Ry sin(wt)
...
% Convert simulated and reference trajectories to 12-state (Euler) for visualization 
...
sgtitle(sprintf('Spiral tracking  |  max error = %.2e m', max(pos_err)));
...
ax3 = axes(fig3);
PlotDrone.setup_axes(ax3, '3D Drone — Spiral Trajectory');
hold(ax3, 'on');
