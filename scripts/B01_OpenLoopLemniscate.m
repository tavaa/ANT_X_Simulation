%% Test open-loop for the quadcopter in a Lemniscate Trajectory — B01 model.
%
...
quad = B01_ANTX_quadcopter(mp, false);
...
sgtitle(sprintf('Lemniscate tracking (rigid body)  |  max pos error = %.2e m', max(pos_err)));

%% FIGURE 1b — MOTOR THRUST STATES (13-16), shows the delay-induced lag
T_hover_pm = mp.m * mp.g / 4;
if quad.delay_motors
    figure('Name', 'Lemniscate Motor Thrusts', 'Color', 'w', 'Position', [30 950 1200 350]);
...
ax3 = axes(fig3);
PlotDrone2.setup_axes(ax3, '3D Drone — Lemniscate Trajectory (B01)');
hold(ax3, 'on');
