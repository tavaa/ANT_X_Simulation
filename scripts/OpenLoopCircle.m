%% Test open-loop for the quadcopter in a Circular Trajectory.
%
...
%       - constant tilt
%       - constant altitude
%       - yaw rotating with angular speed w
%       - nonzero roll/pitch depending on yaw orientation
%
%  Frames:
%       Inertial : NED
%       Body     : FRD
...
% Extract Euler equivalent for logging
if use_quaternions
...
sgtitle(sprintf('Circle tracking  |  max error = %.2e m', max(pos_err)));
...
%% FIGURE 4 — 3D DRONE ANIMATION [1]
fig3 = figure('Name', '3D Circle Animation', 'Color', 'w', 'Position', [900 100 800 750]);
ax3 = axes(fig3);
PlotDrone.setup_axes(ax3, '3D Drone — Circular Trajectory');
hold(ax3, 'on');
