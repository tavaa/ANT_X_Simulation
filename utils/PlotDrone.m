...
%       left motors  : M3-M2
%
% Motor numbering used in THIS code:
%   M1 : front-right  (+x_B,+y_B)
%   M2 : rear-left    (-x_B,-y_B)
%   M3 : front-left   (+x_B,-y_B)
%   M4 : rear-right   (-x_B,+y_B)
%
% Usage:
%   h = PlotDrone.init(ax, mp);
%   PlotDrone.update(h, pos_NED, R);
    ...
    methods (Static)
        ...
        %   Returns struct h with all graphic handles.
            ...
            fx = PlotDrone.FRAME_HX;
            fy = PlotDrone.FRAME_HY;
            fz = PlotDrone.FRAME_HZ;
            ...
            L = PlotDrone.ARROW_LEN;
            h.xb_arr = quiver3(ax, 0,0,0, L,0,0, 0, ...
                'Color', PlotDrone.COL_XB, 'LineWidth', 2.0, 'MaxHeadSize', 0.5);
            h.yb_arr = quiver3(ax, 0,0,0, 0,L,0, 0, ...
                'Color', PlotDrone.COL_YB, 'LineWidth', 2.0, 'MaxHeadSize', 0.5);
            h.zb_arr = quiver3(ax, 0,0,0, 0,0,L, 0, ...
                'Color', PlotDrone.COL_ZB, 'LineWidth', 2.0, 'MaxHeadSize', 0.5);

            % FRD axis labels 
            h.xb_lbl = text(ax, NaN,NaN,NaN, 'x_B (F)', ...
                'Color', PlotDrone.COL_XB, 'FontSize', 7, 'FontWeight','bold');
            h.yb_lbl = text(ax, NaN,NaN,NaN, 'y_B (R)', ...
                'Color', PlotDrone.COL_YB, 'FontSize', 7, 'FontWeight','bold');
            h.zb_lbl = text(ax, NaN,NaN,NaN, 'z_B (D)', ...
                'Color', PlotDrone.COL_ZB, 'FontSize', 7, 'FontWeight','bold');

            % legend patch (invisible anchor for legend) 
            h.leg_xb = plot3(ax,NaN,NaN,NaN,'-','Color',PlotDrone.COL_XB,'LineWidth',2);
            h.leg_yb = plot3(ax,NaN,NaN,NaN,'-','Color',PlotDrone.COL_YB,'LineWidth',2);
            h.leg_zb = plot3(ax,NaN,NaN,NaN,'-','Color',PlotDrone.COL_ZB,'LineWidth',2);
